...
# Create and resize trajectory Figure to plot the trajectories on
trajfig = Figure((500, 500))

# Array to keep track of trajectory lines to display
trajjump = SharedArray{Bool}(tdensity * tdensity)
...
for frames in 1:framesRun
    ...
    for i in 1:tdensity*tdensity
        if (trajjump[i] == false)
            GRUtils.plot([txz[i], ptxz[i]], [tyz[i], ptyz[i]])
            end
        end

    while length(trajfig.plots[1].geoms) > trajectoryLength * tdensity * tdensity
        popfirst!(trajfig.plots[1].geoms)
        end

    global mainfig = deepcopy(trajfig)
    gcf(mainfig)
    hold(true)
